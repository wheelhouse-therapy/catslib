...
class Calendar
{
    ...
    function DrawCalendar()
    {
        ...
        $apiKey_file = CATSDIR_CONFIG."google_client_secret.json";
        if(file_exists($acounts_file)){
            $json = json_decode(file_get_contents($acounts_file));
            if(count($json) > 1){
                $s .= "Multiple Google Accounts Detected";
                $act = "";
                foreach($json as $name => $key){
                    ...
                    if($this->oApp->sess->SmartGPC('acount')){
                        $act .= $name;
                        $apiKey_file = CATSDIR_CONFIG.$key;
                    }
                    else{
        ...
        $oGC = new CATS_GoogleCalendar($apiKey_file);               // for appointments on the google calendar
